...
for i in trange(1000):
    net.load_state_dict(torch.load("../drive/MyDrive/pruning_results/original_trained_network.pt"))
    model_copy = get_cifar_vgg(10, pretrained=False)
    strategy = np.random.uniform(
        0.3, 0.9, size=num_prunable_layers(model_copy)).tolist()
    results_file_name = f"../drive/MyDrive/pruning_results/{str(uuid.uuid1())}.json"
    try:
